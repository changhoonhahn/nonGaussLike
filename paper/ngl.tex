...
\emph{approach} a Gaussian distribution. In practice with finite survey 
volume, shot noise, and systematic effects 


the Gaussian likelihood assumption cannot be true for the powerspectrum. 
In fact, in low signal-to-noise regimes we explicitly expect the 
assumption to fail. \cite{scoccimarro2000} provides a clear 
demonstration of the \todo{finish this sentence}. 
...
which we calculate as $\hat{D}(\Xref \parallel \Yref)$, where $\Xref$ is a 
...
therefore we resample $\Yref$ and compute each of them \todo{100} times. 
The distributions in Figure~\ref{fig:div_gauss} are the $\hat{D}$ 
distributions, which illustrate the uncertainty of $\hat{D}$. The 
discrepancy between the $\hat{D}(\, p(x) \parallel \mathcal{L}^\mathrm{pseudo})$
distributions and the reference $\hat{D}(\Xref \parallel \Yref)$ distribution 
($\Delta \hat{D}$) implies that the pseudo-likelihood is not an accurate estimate of the 
likelihood. Each panel of Figure~\ref{fig:div_gauss} show significant 
discrepancy between the two distributions. In other words, \emph{both 
the $P_\ell(k)$ and $\zeta(N)$ likelihoods are significantly non-Gaussian}. 

Although significant, the discrepancies between the $\hat{D}$ distributions 
for $P_\ell$ are relatively small. This is expected since, as we mentioned 
earlier, the Gaussian pseudo-likelihood assumption for $P_\ell$ is motivated 
by the Central Limit Theorem. As long as enough modes contribute to the 
powerspectrum, the likelihood approaches a Gaussian. In the restrictive 
$k$ range of the \Beut~analysis ($0.01 < k < 0.15$ for $\ell = 0$ and $2$; $0.01 < k < 0.15$ for $\ell = 4$), 
one would expect this to mostly be the case. In order to better understand 
the source of non-Gaussianity, we repeat the divergence comparisons for 
different $k$ ranges. When we set $k_\mathrm{min} = 0.05$, $\Delta \hat{D}$ 
decreases. Meanwhile, when we set $k_\mathrm{max} = 0.1$ for the monopole 
and quadrupole, $\Delta \hat{D}$ increases. Next, if we restrict 
the divergences to only the monopole and quadrupole, $\Delta \hat{D}$ decreases. 
This suggests that $P_\ell$ on large scales (low $k$) and the hexadecapole 
contribute most to the non-Gaussianity of the $P_\ell$ likelihood. 

For $\zeta$, we find relatively large discrepancies between the $\hat{D}$ 
distributions. This is consistent with the fact that the true 
$\zeta$ likelihood distribution is likely Poisson, not Gaussian, 
similar to the likelihood of observed cluster counts~\citep{cash1979,planckcollaboration2014,ade2016}. 
...
the exact $\zeta$ likelihood function tremendously difficult. Nevertheless, 
we expect the likelihood to be non-Gaussian. We again repeat the 
divergence comparison using different $N$ ranges to better understand 
the source of non-Gaussianity. When we exclude the highest $N$ bin, 
$\Delta \hat{D}$ decreases significantly. On the other hand, 
$\Delta \hat{D}$ is not significantly impacted when we exclude
the lowest $N$ bin. High richness groups contibute most to the 
non-Gaussianity of the $\zeta$ likelihood.
...
parameter. Although using the pseudo-likelihood noticeably biases 
the $f \sigma_8$ constraint, the impact relative to its uncertainty 
is small --- less than $0.5 \sigma$. In retrospect, the fact that 
the $P_\ell$ analysis is largely unaffected by likelihood 
non-Gaussianity is not a big surprise. It's consistent with the divergences in 
...
instance, seeks to constrain $f\sigma_8$ to within a percent 
\footnote{DESI Final Design Report: http://desi.lbl.gov/wp-content/uploads/2014/04/fdr-science-biblatex.pdf}. The future, however, is encouraging in this regard. 

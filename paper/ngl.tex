...
(Section~\ref{sec:div}), the PCA components are uncorrelated 
but~\emph{not necessarily statistically independent}~\citep{hartlap2009}. 
Therefore, instead of PCA, we use Independent Component Analysis (ICA). 
...
Using the improved likelihood estimates from the previous section, we can now 
measure the impact of likelihood non-Gaussianity on the posterior constraints 
of these parameters. The most straightforward approach to do this would be to 
recalculate MCMC samples using our likelihood estimates from Sections~\ref{sec:gmm} 
and~\ref{sec:ica}. While this is relatively doable for the~\Beut~analysis, 
for~\Sinh this is \emph{significantly} more involved. Instead of a perturbation 
...
Instead we use importance sampling to derive the posteriors from our estimated 
likelihood from the MCMC chains that used the pseudo-likelihood. Let 
$p_O({\bf x} | \bm{\theta})$ be the original pseudo-likelihood and 
$p_N({\bf x} | \bm{\theta})$ be our ``new'' likelihood. Then the new marginal 
posterior is
\begin{align}
p(\theta_1 | {\bf x}) &= \int p_N({\bf x} | \bm{\theta})\,\mathrm{d}\theta_2...\mathrm{d}\theta_p \\
&= \int \frac{p_N({\bf x} | \bm{\theta})}{p_O({\bf x} | \bm{\theta})}\, p_O({\bf x} | \bm{\theta})\,\mathrm{d}\theta_2...\mathrm{d}\theta_p \\ 
&= \int \frac{p_N(\bm{\theta} | {\bf x})}{p_O(\bm{\theta} | {\bf x})}\, p_O({\bf x} | \bm{\theta})\,\mathrm{d}\theta_2...\mathrm{d}\theta_p.
\end{align}
Then using Monte Carlo integration, 

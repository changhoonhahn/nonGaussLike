...
Bayesian parameter inferece has become the standard framework for 
deriving cosmological parameters of interest from observed 
measurements such as the galaxy powerspectrum or two-point correlation 
function in large scale scturture (LSS) studies and cosmology analyses 
in general. From Bayes rule, 
\beq
p( \theta\,|\,x) \propto p( x\,|\,\theta) p(\theta).
\eeq
the posterior parameter constraints can be calculated from the prior 
and the likelihood -- probability of the data given the theoretical model.
Since anlayses choose priors, parameter inference ultimately boils down 
to evaluating the likelihood. Hence it goes without say that our analyses can 
only lead to unbiased constraints if the employed likelihood is correct. 
...
form. LSS analyses employ a Gaussian \emph{pseudo}-likelihood. 
\todo{sentence about how this is different than non-Gaussian terms in the covariance matrix}.
Second, the covariance matrix used in evaluating the Gaussian pseudo-likelihood
is assumed to be independent of the parameters $\theta$. \todo{explain 
in more detail}. The dependence of the covariance matrix 
on cosmological parameters has been shown to have a significant effect
on parameter constraints~\citep[\emph{e.g.}][]{eifler2009}. In this paper, 
we focus on evaluating the impact of the first assumption (\emph{i.e.} the 
Gaussian pseudo-likleihood) on cosmological parameter constraints. 

...
from Central Limit Theorem, the likelihood of the powerspectrum would \emph{approach} a Gaussian 
distribution. With finite survey volume, shot noise, and systematic effects 
the Gaussian likelihood assumption cannot be true for the powerspectrum on 
all scales. In fact, for the low signal-to-noise regime (on large scales) we explicitly expect 
the assumption to fail. \cite{scoccimarro2000} provides a clear 
...
talk about CMB likelihood analyses? Bring up sellentin et al.'s t-distribution stuff?  
...
For $\zeta$, we find a relatively large discrepancy between the $\hat{D}$ 
distributions. This is consistent with the expectation that the true 
...
we expect the likelihood to be non-Gaussian. In order to better understand
the source of non-Gaussianity, we repeat the divergence comparison excluding
the highest $N$ bin. We find \todo{something}. When we instead exclude the 
lowest $N$ bin, we find \todo{something}. \todo{conclusion about this}.   

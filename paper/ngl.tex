...
of their model parameters. The \Beut~analysis involves $11$ parameters:
...
The \Sinh~analysis involves $5$ parameters:
$\log\,M_\mathrm{min}$, $\sigma_{\log\,M}$, $\log\,M_0$, $\log\,M_1$, and $\alpha$. 
Using the improved likelihood estimates from the previous section, we now 
measure the impact of likelihood non-Gaussianity on the posterior constraints 
of these parameters. The most straightforward approach would be to replace the 
Gaussian pseudo-likelihood with the likelihood estimates from 
Sections~\ref{sec:gmm} and~\ref{sec:ica} and then regenerate the MCMC 
sample. While this is relatively doable for the~\Beut~analysis, 
...
Instead, without rerunning the MCMC chains, we use importance sampling to 
derive the posteriors from our estimated likelihood. 
Let
\beq
p(\theta_1 | {\bf x}) = \int p(\bm{\theta} | {\bf x})\,\mathrm{d}\theta_2...\mathrm{d}\theta_p
%f(x) \mathrm{d}x = \int \frac{h(x) f(x)}{g(x)} g(x) \mathrm{d}x
\eeq
then 
\beq
\hat{I} = \frac{1}{N} \sum\limits_{i=1}^N \frac{h(x_i) f(x_i)}{g(x_i)}
\eeq

\todo{explain importance sampling} 

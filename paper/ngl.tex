...
Survey~\cite{taylor2015}, \todo{pulsar stuff} \citep{lee2012}
...
In Figure~\ref{fig:gmf_ped}, we illustrate Gaussian Mixture density 
...
Now using the Gaussian mixture density estimation method described above
we can directly estimate the likelihood distribution using the mock catalogs. 
Using the EM algorithm, detailed above, we fit Gaussian mixture models with 
$k < 30$ components to the mock data $\Xmock$. Afterwards, the model with 
the lowest BIC ($k = $\todo{find out k}) is selected as the best density
estimate of the likelihood distribution. This 
...
\begin{figure}
\begin{center}
\includegraphics[width=0.9\textwidth]{figs/GMM_pedagog.pdf}
\caption{}
\label{fig:gmf_ped}
\end{center}
\end{figure}

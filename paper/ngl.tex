...
\begin{abstract}
    \todo{Standard present day LSS analyses makes a major assumption in their parameter 
inference --- the likelihood has a Gaussian functional-form. We 
investigate the impact of this assumption on two recent LSS analyses: 
the \Beut~powerspectrum multipole analysis and the \Sinh~group multiplicity 
function analysis. We combine the mock catalogs, originally constructed for 
covariance matrix estimation in these analyses, with non-parametric 
divergence estimators in order to quantify the non-Gaussianity of the 
likelihood. Afterwards, we use the same mocks and non-parametric density 
estimation to construct likelihood estimates that better approximate 
the non-Gaussian likelihood. These estimates allow us to quantify the 
impact of likelihood non-Gaussianity on the posterior probability 
distributions of the \Beut~and~\Sinh~parameters. Furthermore, since our 
likelihood estimates better approximate the true likelihood, they allow us to derive more 
...
and therefore increase the number of modes on all scales. Thanks to 
the Central Limit Theorm, we consequently expect 
likelihood non-Gaussianity to have an even smaller effect. However, for 
higher order statistics such as the bispectrum, which has lower 
...
Standard present day LSS analyses makes a major assumption in their parameter 
inference --- the likelihood has a Gaussian functional-form. We 
investigate the impact of this assumption on two recent LSS analyses: 
the \Beut~powerspectrum multipole analysis and the \Sinh~group multiplicity 
function analysis. We combine the mock catalogs, originally constructed for 
...
distributions of the \Beut~and~\Sinh~parameters. Furthermore, since our 
likelihood estimates better approximate the true likelihood, they allow us to derive more 
accurate parameter constraints. We find the following results from 
our analysis:  

\begin{itemize}
    \item We measure the divergences between the $P_\ell$ and $\zeta$ 
        likelihoods and their Gaussian pseudo-likelihoods in order to 
        quantify the non-Gaussianity of the likelihoods. The comparisons
        of the divergences reveal significant non-Gaussianity for both 
        the $P_\ell$ and $\zeta$ likelihoods. \todo{Something about how 
        the $\zeta$ likelihood is more non-Gaussian than the $P_\ell$ 
        likelihood}
    \item From the \Beut~and~\Sinh~mocks, we use Gaussian mixture density 
        estimation and independent component analysis to construct 
        estimates of the $P_\ell$ and $\zeta$ likelihoods. The divergence 
        between our estimates and the $P_\ell$ and $\zeta$ likelihoods 
        demonstrate that our estimates more accurately approximate the 
        true likelihoods than the standard pseudo-likelihood. 
    \item 
        Parameter constraint comparison results for \Beut
        Parameter constraint comparison results for \Sinh
\end{itemize}

Nevertheless, the divergence and likelihood 
estimation frameworks we introduce in this paper provides a straightforward 
way of testing and quantifying the impact of likelihood non-Gaussianity. 


methods likelihood-free inference such as Approximate Bayesian 
Computation have garnered popularity in the literature~\citep[\emph{e.g.}][]{hahn2017a}

...
distribution, we use the original posterior as the \emph{proposal} 
distribution with the ratio of our likelihood estimates over the pseudo-likelihood 
...
To derive the importance sampled posterior distributions using 
the non-Gaussian likelihood estimates from this paper, 
$P({\bf x} | \bm{\theta}^{(i)})$ 
is the pseudo-likelihood and 
$P'({\bf x} | \bm{\theta}^{(i)})$
...
confidence interals of the updated and original posteriors. 
...
non-Gaussianity is incorporated. For $f \sigma_8$, the posterior
...
its uncertainty is small. In retrospect, the fact that the $P_\ell$
analysis is largely unaffected by likelihood non-Gaussianity is not a 
big surprise. In fact, its consistent with the divergences in 
...
In dotted lines, we plot the 
original \Sinh~posteriors, which differ from the blue distribution.
...
that \emph{due to likelihood non-Gaussianity, using the Gaussian 
pseudo-likelihood significantly underestimates the uncertainty and 
biases the parameter constraints of the \Sinh~$\zeta$ analysis.}

\todo{Need to talk about what causes the likelihood non-Gaussianity}
...
\section{Summary}
Future surveys such as the Dark Energy Spectroscopic Instrument 
\todo{(citecite)}, will expand the cosmic volumes probed by galaxies 
and therefore the increase number of modes on all scales. Consequently,
for $P_\ell$ analyses we expect likelihood non-Gaussianity to have 
an even smaller effect using these surveys. For higher order statistics 
such as the bispectrum, however, likelihood non-Gaussianity 


One of the main results of \Sinh, is that while the $\Lambda$CDM + HOD 
model is successful at fitting either $\zeta(N)$ or the project two-point
correlation function $w_p(r_p)$, it struggles to jointly fit both 
observables. Such tensions suggests shortcomings in the `vanilla' 
HOD model. However, once likelihood non-Gaussianity is 
correctly considered, the posteriors are broadened and shifted to relax 
these tensions. We examine the effect of likelihood non-Gaussianity 
in more detail in Hahn et al. (in prep.). 

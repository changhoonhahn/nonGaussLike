...
underestimates the uncertainty and biases the parameter constraints 
of the \Sinh~$\zeta$ analysis.}

The contrast between the pseudo-likelihood posteriors and our
posteriors appropriately reflect the divergences in 
Figure~\ref{fig:div_gauss}, which revealed significant
...
algorithm and the underlying matter overdensity makes  
writing down the exact $\zeta$ likelihood function 
would be tremendously difficult. Regardless, the GMM likelihood 
estimation method we present provides a straightforward estimate 
of the non-Gaussian likelihood. 

The posteriors of the \Sinh~$\zeta$ analysis, highlights the 
significance of likelihood non-Gaussianity on parameter inference 
in LSS studies. One of the main results of the \Sinh~HOD analysis, 
...
Even for the $P_\ell$ 
analysis, the impact of likelihood non-Gaussianity on the parameter constraints cannot be dismissed  
as we demand increasingly more precise constraints.  
\todo{maybe some handwavy back of the envelope argument with DESI?} 
...
non-Gaussianity 
Fortunately the methods we present in this paper make it 
straightforward to quantify likeilhood non-Gaussianity and 
examine the effect on parameter inference. 
...
Standard LSS analyses make two key assumptions in their parameter 
inference. 
First, they assume that the likelihood has a Gaussian functional form. 

...
\begin{abstract}
    ...
    the \cite{sinha2017}~group multiplicity function ($\zeta$) analysis. For both 
    $P_\ell$ and $\zeta$ likelihoods, we identify significant non-Gaussianity with 
    non-parametric divergence estimators used in Machine Learning on mock catalogs 
    originally constructed for covariance matrix estimation. We then use Gaussian mixture 
    ...
    of \Beut~and~\Sinh. For \Beut, likelihood non-Gaussianity shifts the $f\sigma_8$ 
    constraint by $0.44\sigma$, but otherwise, does not significantly impact the 
    overall parameter constraints. For the $\zeta$ analysis, using the pseudo-likelihood 
    significantly underestimates the uncertainties and biases the constraints of 
    \Sinh~halo occupation parameters: $\log\,M_\mathrm{min}$, $\log\,M_1$ and $\alpha$. 
    The divergence and likelihood estimation methods we present provides a straightforward 
    framework for quantifying the impact of likelihood non-Gaussianity and deriving more 
...
%observed measurements such as the galaxy powerspectrum or two-point correlation function in large scale scturture (LSS) studies and cosmology analyses in general. 
Using Bayes' rule, 
\beq
...
and \cite{sinha2017} (hereafter~\Sinh). \Beut~analyzes the powerspectrum 
...
can be used to quantify the non-Gaussianity. More importantly we will use the 
mocks to estimate the ``true'' non-Gaussian likelihood. 
...
accuracy. We utilize an addition $99$ sets of HOD parameters of $200$
mocks each. The HOD parameters are sampled from the MCMC chain used to 
...
($\ell = 0, 2, 4$) of the powerspectrum using FFTs of the 
...
powerspectrum is computed in bins of $\Delta k = 0.01\,h\,\mathrm{Mpc}^{-1}$
...
$D(\, p(x) \parallel \mathcal{L}^\mathrm{pseudo}) \approx \hat{D}(\Xmock \parallel \mathcal{L}^\mathrm{pseudo})$. %\mathcal{N}(\overline{\bf X}^\mathrm{mock}, \mathbb{C}\,))$.
...
In Figure~\ref{fig:div_gauss}, we present the resultings distributions of $\hat{D}$,
...
exclude the largest scales and set 
$k_\mathrm{min} = 0.05$, $\Delta \hat{D}$ decreases. Meanwhile, if we 
exclude the smallest scales and set $k_\mathrm{max} = 0.1$ for all 
...
\includegraphics[width=0.9\textwidth]{figs/kNNdiverg_Gauss.pdf}
    ...
    $\hat{D}_{KL}$ \Nd times and plot their distribution 
    in order to illustrate the uncertainty of the $\hat{D}$ 
...
selected. The choice of $k=3$ here is obviously justified. The $k = 1$ 
GMM is definitely not flexible enough to characterize the underlying 
distribution and the $k=10$ GMM clearly overfits the distribution. In 
this example, GMM with $k$ selected by BIC correctly identifies the three 
Gaussian distributions from which the data is drawn. 
...
the same approach as our $\hat{D}(\Xmock | \Yref)$ calculation in Section~\ref{sec:div}. 
...
we repeat this process \Nd times, resampling $\hat{p}_\mathrm{\tiny GMM}$
...
$\hat{p}_\mathrm{\tiny GMM}$ and the likelihood distribution in purple 
for the $P_\ell(k)$ (top) and $\zeta(N)$ (bottom) analyses. For comparison, 
we include the $\hat{D}$ distributions from Figure~\ref{fig:div_gauss}. 
...
\caption{R\'enyi-$\alpha$ and KL divergence estimates 
    ($\hat{D}_{R\alpha}$ and $\hat{D}_{KL}$; purple) between 
...
\caption{R\'enyi-$\alpha$ and KL divergence estimates 
    ($\hat{D}_{R\alpha}$ and $\hat{D}_{KL}$; green) between 
    ...
    provides a significant improvement. Our ICA likelihood provides
    a significantly better estimate of the true $P_\ell$ likelihood
    than the pseudo-likelihood.
    }
\label{fig:div_ica}
\end{center}
...
increase the size of their observable data vector. As 
...
sections we use the ICA likelihood for the~\Beut~analysis and the GMM 
likelihood for the~\Sinh~analysis. 
...
MCMC samples from scratch. While this is relatively tractable for the~\Beut~analysis, 
...
the remarkable effectiveness of the Central Limit Theorem. %for the powerspectrum multipole over the \Beut~$k$ range. 
...
\begin{tabular}{cccccc} \toprule
    ...
    \multicolumn{1}{c}{$1.351^{+}_{-}$} & 
    \multicolumn{1}{c}{$1.335^{+}_{-}$} &
    \multicolumn{1}{c}{$1.293^{+}_{-}$} & 
    \multicolumn{1}{c}{$0.422^{+}_{-}$} &
    \multicolumn{1}{c}{} \\
    \multicolumn{1}{c}{$\mathcal{L}_\mathrm{ICA}$} & 
    \multicolumn{1}{c}{$0.456^{+}_{-}$} & 
    \multicolumn{1}{c}{$1.001^{+}_{-}$} & 
    \multicolumn{1}{c}{$1.014^{+}_{-}$} & 
    \multicolumn{1}{c}{} & 
...
the divergences reveal 
significant likelihood non-Gaussianity. For the $P_\ell$ likelihood,
...
significantly biases their constraints. Accounting for likelihood 
...
highlight the importance of incorporating likelihood non-Gaussianity in 
...
range, however, likelihood non-Gaussianity may still be a significant effect. 
For higher order statistics such as the galaxy bispectrum or three-point 
function, even for future surveys, likelihood non-Gaussianity 
will likely be a significant effect to consider for parameter inference.  

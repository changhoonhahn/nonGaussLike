...
component analysis~\citep[PCA;][]{Press:1992:NRC:148286} to 
transform the problem into a basis where the problem can be 
decomposed into multiple one dimensional problems. If the likelihood
is Gaussian, the PCA components of it are statistically independent. 
...
Therefore, we cannot use PCA; instead, we use Independent Component Analysis 
(ICA) following \cite{hartlap2009}. 

We begin with the assumption that the observable ${\bf x}$ 
(\emph{e.g.} $P_\ell$) is generated by some linear transformation
${\bf M}$ of ${\bf x}^\mathrm{IC}$, a vector of statistically independent 
source signals with non-Gaussian probability distributions: 
\beq \label{eq:ica0} 
{\bf x} = {\bf M}\,{\bf x}^\mathrm{IC}.
\eeq
Then the goal of ICA is to estimate both ${\bf M}$ and 
${\bf x}^\mathrm{IC}$. Since each component of ${\bf x}^\mathrm{IC}$ 
is statistically independent, the distribution of the observable 
(the likelihood) then becomes the product of one dimensional distributions: 
...
The basic premise of ICA is simple, maximizing non-Gaussianity
maximizes the statistical independence. Consider a single component 
of some linear combination of ${\bf x}$, ${\bf y} = {\bf W}\,{\bf x}$: 
\beq
{\bf y}_n = {\bf w}_n^{\tiny t}\,{\bf x} 
\eeq
where ${\bf w}_n^{\tiny t}$ is the $n^\mathrm{th}$ row of ${\bf W}$. 
Plugging in Eq.~\ref{eq:ica0} this becomes
\beq
{\bf y}_n = {\bf w}_n^{\tiny t}\,{\bf M}\,{\bf x}^\mathrm{IC}. 
\eeq
...
${\bf x}^\mathrm{IC}$ components. 


\todo{Although we refer readers to~\cite{hyvarinen2001independent} for
a more rigorious justification of ICA, the basic premise of it 
is}

...
\author{
    ChangHoon~Hahn\altaffilmark{\lbl},
...
structure (\lss) studies. {\sout{and cosmology in general}}. 
%observed measurements such as the galaxy power spectrum or two-point correlation function in large scale scturture (\lss) studies and cosmology analyses in general. 
Using Bayes' rule, 
\beq
...
reduces to evaluating the likelihood. Consequently,   
analyses can yield unbiased constraints {\em only} if the likelihood
...
%% p(x\,|\,\theta) = \frac{1}{(2\pi)^{d/2}
%%   \sqrt{\mathrm{det}(C(\theta))}}\,\,\mathrm{exp}\bigg[\frac{1}{2}(x -
%%   m(\theta))^{t} C(\theta)^{-1} (x - m(\theta))\bigg]
...
acoustic oscillation scale. Meanwhile \Sinh~fits to the observed SDSS group multiplicity 
...
analyses of \lss studies. Mocks are used for testing analysis 
...
In order to correctly estimate the covariance matrices 
...
$N_\mathrm{mock} = 20,000$ mocks. \ms{Since the covmat `C' depends on $\theta$,
  how does averaging over 20000 mocks with different HOD parameters differ
  from 20000 mocks from the same HOD parameter?}
...
$D(\, p(x) \parallel \mathcal{L}^\mathrm{pseudo}) \approx \widehat{D}(\Xmock \parallel \mathcal{L}^\mathrm{pseudo})$. %\mathcal{N}(\overline{\bf X}^\mathrm{mock}, \mathbb{C}\,))$.
...
non-Gaussianity on the final cosmological parameter constraints; and then
to develop more accurate methods for parameter inference in \lss. 
To determine the `true' cosmological parameter constraints, we first need
to construct a better likelihood estimator, i.e., a non-Gaussian likelihood
estimator. 
%% From the divergence estimates alone, it is not obvious how they propagate 
%% onto the final parameter constraints.
In this section, we present two methods for more accurately estimating the true
non-Gaussian likelihoods of $P_\ell$ and $\zeta$ from their corresponding mocks.
Moreover, we will use them later to quantify the impact of likelihood non-Gaussianity on the 
...
%% The selected density estimate can 
...
%% confirm whether $\widehat{p}_\mathrm{\tiny GMM}$ is in fact a better estimate 
...
pseudo-likelihood for the $\zeta(N)$ analysis of \Sinh. In other words, \emph{our Gaussian mixture density estimate  
is a significant better estimate of the $\zeta$ likelihood distribution
than the pseudo-likelihood}.\ms{I see that the purple is closer to the
      orange but there is still a big gap -- can we justify the ``significantly
      better estimate''?} On the other hand, the Gaussian mixture 
...
Since the large divergence discrepancy in the $P_\ell$ likelihood indicates
that the Gaussian mixture density estimate of the non-Gaussian
likelihood can potentially be improved by using a different estimator. 
In the next section we present an alternative method for estimating the
non-Gaussian likelihood using Independent Component Analysis.
...
\caption{R\'enyi-$\alpha$ and KL divergence estimates 
    ...
    (bottom) analyses. We include the divergence estimates for Gaussian
    pseudo-likelihood from 
...
For \emph{both} \Beut~and~\Sinh, our \ica likelihood significantly \ms{As
  before, can we justify/quantify the ``significantly''?}
...
Since the increasing size of the observable data vector places strong
requirements for larger sets of mock catalogs, the effectiveness of the \ica method in estimating
higher dimensional likelihoods with fewer samples (mocks) is  
particularly appealing. 
\cite{hartlap2009} suggest that a low $N_\mathrm{mock}$ may bias the 
\ica likelihood estimate. By examining the divergence discrepancy
from figures like Figures~\ref{fig:div_ica} and~\ref{fig:div_gmm}, we can 
pinpoint likelihood estimation methods that provide a better estimate of the true likelihood than the 
Gaussian pseudo-likelihood. We do not even need to make a single choice for the 
estimator -- the divergence estimation framework makes it
straight-forward to test the accuracy of different methods. Multiple methods can easily be tested to construct 
...
Therefore, we use importance sampling to derive the new posteriors
...
likelihood. \ms{What does the \ica estimator look like for \Sinh? Or the \gmm
  estimator for \Beut?}
...
%, which found relatively small discrepancies 
%between the likelihood and pseudo-likelihood.
It also illustrates
the remarkable effectiveness of the Central Limit Theorem. %for the power spectrum multipole over the \Beut~$k$ range. 
...
%to the power spectrum, then the likelihood approaches a Gaussian. In 
...
%% The future, however, may be encouraging in this regard.
...
%% likelihood non-Gaussianity to have a smaller effect.
%% However, without precisely quantifying the impact, as we have done in this paper, it 
...
Although this Gaussian assumption is motivated by the Central Limit Theorem, 
the assumption is not necessarily true in detail. 
...
analysis and the \Sinh~group multiplicity function analysis. Using mocks from
each of the original analyses, we measure the
divergences between the $P_\ell$ and $\zeta$
likelihoods and the Gaussian pseudo-likelihoods from \Beut~and~\Sinh.
%% We combine 
%% mock catalogs originally constructed 
%% for covariance matrix estimation in these analyses, with non-parametric 
%% divergence estimators used in Machine Learning, to measure the divergences 
%% between the $P_\ell$ and $\zeta$ likelihoods and the Gaussian 
%% pseudo-likelihoods from \Beut~and~\Sinh.
For both the $P_\ell$ and $\zeta$ likelihoods, 
...
In both analyses, we find that the low signal-to-noise regime contributes the most to the 
...
{\em assumed} Gaussian pseudo-likelihoods. 
...
respectively. Accounting for likelihood non-Gaussianity \ms{Same here: we need
  to soften this statement} significantly 
eases the tension between the $\zeta$ and $w_p(r_p)$ constraints found 
...
associated future analyses will attempt to extend the $k$ ranges probed without compromising 
constraining power. Since future surveys with larger cosmic volumes will yield
a larger number of high multiplicity groups, the $\zeta$ likelihood will be
more Gaussian, thereby easing the requirements for adopting a non-Gaussian likelihood. 
However, likelihood non-Gaussianity may still be a significant effect for a wider multiplicity range. For higher order statistics 
such as the galaxy bispectrum or the three-point function 
likelihood non-Gaussianity will still likely be a significant effect to consider for 
parameter inference even for larger future surveys. 
Since primordial non-Gaussianity ($f_\mathrm{NL}$) constraints from \lss derive
most of their constraining power from the most (i.e., the largest scales with
the fewest modes) non-Gaussian scales, we expect likelihood
non-Gaussianity will have a significant impact. 
Regardless of expectation, for accurate parameter inference the Gaussian likelihood assumption must 
be extensively tested. The divergence and likelihood estimations we introduce in 
this paper such a framework for testing and quantifying the 

...
\begin{abstract}
    ...
    the \cite{sinha2017}~group multiplicity function ($\zeta$) analysis. Using non-parametric 
    ...
    constraint by $0.44\sigma$. Otherwise, likelihood non-Gaussianity does not 
    significantly impact the overall \Beut~parameter constraints. For the $\zeta$ analysis, 
...
from the Central Limit Theorem. Let's take the powerspectrum of the density 
field as an example. Assuming that the density field is a Gaussian random field, 
which it approximately is on large scales, the powerspectrum of a specific 
Fourier mode would follow a chi-squared distribution, \emph{not} a Gaussian. 
With sufficiently many independent modes contributing, however, by the Central 
Limit Theorem the likelihood of the powerspectrum would \emph{approach} a 
Gaussian distribution. That means in practice we expect the Gaussian assumption 
to fail in low signal-to-noise regimes. The assumption is further invalidated 
...
Even if the likelihood were Gaussian, \cite{sellentin2016} argue that 
since an estiamte of the covariance matrix is used for the likelihood, 
...
for more precise parameter constraints from LSS, analyses must go beyond the 

...
acoustic oscillation scale. Meanwhile \Sinh~analyzes the group multiciplicty 
...
implies that the pseudo-likelihood is not an accurate estimate of the 
...
For $P_\ell$, although we find significant discrepancies between the 
$\hat{D}$ distributions, the discrepancy is relatively small. This is 
expected since, as we mentioned earlier, the Gaussian pseudo-likelihood 
assumption for $P_\ell$ is motivated by the Central Limit Theorem. As 
long as enough modes contribute to the powerspectrum, the likelihood 
approaches a Gaussian. In the restrictive $k$ range of the \Beut~analysis 
($0.01 < k < 0.15$ for $\ell = 0$ and $2$; $0.01 < k < 0.15$ for $\ell = 4$), 
one would expect this to be the case. When we repeat the divergence 
comparison with $k_\mathrm{min} = 0.05$, we find \todo{something}. On 
the other hand when we repeat the comparison for $k_\mathrm{max} = 0.1$,
we find \todo{something}. \todo{what conclusion do we draw from this}.

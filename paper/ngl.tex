...
In the following sections, based on the performances of the GMM and ICA 
...
the original~\Beut~posterior distributions for comparison in 
blue. On the bottom of each panel, we include box plots marking 
the confidence interals of the updated and original posteriors. 
The boxes and ``whiskers'' repesent the $68\%$ and $95\%$ 
confidence intervals, respectively. We also list the median
and $68\%$ confidence intervals of the posteriors in Table~\ref{tab:posterior}.
Overall, likelihood non-Gaussianity does \emph{not} have a significant
impact on the parameter constraints of the $P_\ell$ analysis. $f \sigma_8$ 
and $b_2 \sigma_8$ are the main parameters with noticeable 
change in their posteriors. \todo{put the actual numbers} 
...
We also list the median and $68\%$ confidence intervals 
in Table~\ref{tab:posterior}. In dotted lines, we plot the 
...
\emph{Likelihood non-Gaussianity 
significantly impacts the posterior distribution of all the HOD 
parameters except $\log M_0$}.
\todo{put in details once the numbers out}
The impact of likelihood non-Gaussianity is further emphasized in 
the joint posterior distributions of Figure~\ref{fig:gmf_contour}.
The $(\log\,M_\mathrm{min},\,\sigma_{\log M})$ and 
$(\log\,M_\mathrm{min},\,\alpha)$ contours are both shifted 
and broadened. 

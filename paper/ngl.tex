...
\todo{add here how some of this discrepancy is not necessarily from non-Gaussianity
but could also be from not enough mocks to constrain all the terms of the covariance matrix} 
...
posterior. \todo{add: With 20,000 mocks and only 8 dimensional data, 
this discrepancy is certainly not from convergence issues.}
Figures~\ref{fig:gmf_like}~and~\ref{fig:gmf_contour}~reveal 
...
\todo{add: While some of this discrepancy might just be statistical 
fluctuations, since the mocks are calibrated so that their mean powerspectrum 
is consistent with the BOSS $P(k)$, this should not be a significant contribution}
